...
Create a Student class that extends the Person
 class. Student class has "year", "onEnroll" and "grade" attributes.
Give the student class default attributes: year 
is -1, onEnroll is an empty arrow function, grade is -1.
Student class has a getNotified(success)
 function that console.logs the student name,
 and then calls the onEnroll() function, 
 if success is equal to true.
Make an **Academy** class that has a "name" attribute,
 and a "students" attribute. 
 The name by default is 
 "Default", and "students" is an empty array.
Add students in the Academy class using a 
function called **join(student)**, which adds
 the student to the students array and calls the ".getNotified(true)" function of the student.
